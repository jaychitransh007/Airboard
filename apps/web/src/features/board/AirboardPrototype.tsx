...
import {
  ...
  type RealtimeSpeechTranscriptEvent,
...
// (or within its short grace window after closing, so a transcript that
// finalizes just after release still lands), finalized speech routes straight
// to the command pipeline without the "Airo" wake word. Ambient meeting talk
// stays inert because no gate is open.
type VoiceGateState =
  | { mode: "ptt"; openedAt: number; closedAt: number | null }
  | { mode: "scoped"; strokeId: string; openedAt: number; closedAt: number | null };

type VoiceGateUi =
  | { mode: "ptt" }
  | { mode: "scoped"; strokeId: string; label: string };

/** Palm push-to-talk pose thresholds (score from estimatePalmPresentation). */
const PALM_GATE_ENGAGE_SCORE = 0.62;
const PALM_GATE_RELEASE_SCORE = 0.45;
const PALM_GATE_ENGAGE_MS = 280;
const PALM_GATE_RELEASE_MS = 240;
/** Normalized camera-space drift allowed while the pose "holds still". */
const PALM_GATE_STILLNESS_RADIUS = 0.06;
/** A final transcript may land shortly after the palm drops. */
const PTT_GATE_GRACE_MS = 1_600;
/** Hold-to-edit: grab an element and keep it still this long to scope it. */
const SCOPED_HOLD_MS = 600;
const SCOPED_HOLD_MOVE_TOLERANCE_PX = 14;
/** The scoped mic lingers after release so "grab, let go, speak" works. */
const SCOPED_GATE_GRACE_MS = 8_000;
 ...
 */
function boardContentChanged(a: BoardState, b: BoardState): boolean {
...
export function AirboardPrototype({ surface }: { surface: Surface }) {
  ...
  const palmGateTrackerRef = useRef<{
    candidateSince: number | null;
    anchor: { x: number; y: number } | null;
    belowSince: number | null;
  }>({ candidateSince: null, anchor: null, belowSince: null });
  const grabHoldRef = useRef<{
    strokeId: string;
  ...
  const openVoiceGate = useCallback((gate: VoiceGateState) => {
    ...
    const label =
      boardRef.current.strokes[gate.strokeId]?.annotation?.label?.trim() || "this object";
    setVoiceGate({ mode: "scoped", strokeId: gate.strokeId, label });
    setCommandFeedback(
      micArmed
  ...
  const closeVoiceGate = useCallback((mode: VoiceGateState["mode"]) => {
    const gate = voiceGateRef.current;
    if (!gate || gate.mode !== mode || gate.closedAt !== null) {
      return;
    }
    gate.closedAt = Date.now();
    if (mode === "ptt") {
      // The scoped pill stays visible through its grace window ("grab, let
      // go, speak"); push-to-talk reads as released the moment the palm drops.
      setVoiceGate(null);
    }
  }, []);

  /**
   * Explicit-channel routing: a finalized transcript spoken while a voice gate
   * is open (or within its grace window) becomes a command directly — no wake
   * word. Returns false when no gate applies so the wake-word router runs.
   */
  const routeGatedTranscript = useCallback(
    (
      transcript: string,
      metadata: {
      ...
      if (gate.closedAt !== null) {
        const grace = gate.mode === "scoped" ? SCOPED_GATE_GRACE_MS : PTT_GATE_GRACE_MS;
        if (now - gate.closedAt > grace) {
      ...
      // A gated utterance may still carry a habitual "Airo" prefix — strip it
      // rather than parsing the wake word as command text.
  ...
  const startVoiceCommand = useCallback(() => {
    ...
    if (speechEngine === "realtime") {
      ...
      let activeFinalEvent: RealtimeSpeechTranscriptEvent | null = null;
      const commandRouter = createBrowserWakeCommandRouter({
        onWake: () => {
          setSpeechRecognitionStatus("wake-detected");
        ...
        onCommand: ({ command, transcript, wakePhrase }) => {
          ...
          queueVoiceCommand(command, {
            ...
            wakePhrase,
            engine: "realtime",
            provider: activeFinalEvent?.provider ?? metadata?.provider,
            model: activeFinalEvent?.model ?? metadata?.model,
            confidence: activeFinalEvent?.confidence,
            turnIndex: activeFinalEvent?.turnIndex,
          });
        },
      });
      ...
      realtimeSession = createRealtimeSpeechSession(
        {
          ...
          onFinal: (transcript, event) => {
            const metadata = realtimeSession?.getMetadata();
            // Explicit gates (palm push-to-talk, hold-to-edit) outrank the
            // wake-word router: while one is engaged the user is unambiguously
            // addressing the board.
            if (
              routeGatedTranscript(transcript, {
                engine: "realtime",
            ...
            const routedBefore = routedCommandCount;
            activeFinalEvent = event;
            commandRouter.consumeFinalTranscript(transcript);
            activeFinalEvent = null;
            reportFinalTranscript(transcript, routedCommandCount > routedBefore, {
              engine: "realtime",
              provider: event.provider ?? metadata?.provider,
              model: event.model ?? metadata?.model,
              confidence: event.confidence,
    ...
    const browserSession = createBrowserWakeSpeechSession({
      ...
      onHeard: ({ transcript, isFinal }) => {
        ...
        // The fallback session routes wake-word commands internally, so the
        // gate only claims transcripts the wake router would ignore — a gated
        // "Airo …" utterance still routes exactly once (via the router).
        if (
          classifyBrowserWakeTranscript(transcript).commands.length === 0 &&
          routeGatedTranscript(transcript, {
  ...
  const updatePalmPushToTalk = useCallback(
    (hands: readonly DetectedHand[], timestampMs: number) => {
      const tracker = palmGateTrackerRef.current;
      let bestScore = 0;
      ...
      const gate = voiceGateRef.current;
      const pttOpen = gate?.mode === "ptt" && gate.closedAt === null;

      if (bestScore >= PALM_GATE_ENGAGE_SCORE && bestPoint) {
        tracker.belowSince = null;
        if (
          tracker.candidateSince === null ||
          !tracker.anchor ||
          Math.hypot(bestPoint.x - tracker.anchor.x, bestPoint.y - tracker.anchor.y) >
            PALM_GATE_STILLNESS_RADIUS
        ) {
          tracker.candidateSince = timestampMs;
          tracker.anchor = bestPoint;
          return;
        }
        if (
          !pttOpen &&
          !(gate?.mode === "scoped" && gate.closedAt === null) &&
          timestampMs - tracker.candidateSince >= PALM_GATE_ENGAGE_MS
        ) {
          openVoiceGate({ mode: "ptt", openedAt: Date.now(), closedAt: null });
        }
        return;
      }
      ...
      if (bestScore >= PALM_GATE_RELEASE_SCORE) {
        // Hysteresis band: an engaged gate survives a briefly weaker pose.
        tracker.belowSince = null;
        return;
      }
      if (tracker.belowSince === null) {
        tracker.belowSince = timestampMs;
        return;
      }
      if (timestampMs - tracker.belowSince >= PALM_GATE_RELEASE_MS) {
        tracker.belowSince = null;
        closeVoiceGate("ptt");
      }
    },
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      const interaction = objectInteractionRef.current;
      if (interaction?.mode === "moving") {
        const point = lastGesturePointerRef.current;
        const hold = grabHoldRef.current;
        if (!hold || hold.strokeId !== interaction.strokeId) {
          grabHoldRef.current = point
        ...
        if (
          Math.hypot(point.x - hold.anchor.x, point.y - hold.anchor.y) >
          SCOPED_HOLD_MOVE_TOLERANCE_PX
        ) {
          hold.moved = true;
          return;
        }
        if (now - hold.startedAt >= SCOPED_HOLD_MS) {
          const current = voiceGateRef.current;
          const scopedAlready =
            current?.mode === "scoped" &&
            current.closedAt === null &&
            current.strokeId === interaction.strokeId;
          if (!scopedAlready) {
            openVoiceGate({
              mode: "scoped",
              strokeId: interaction.strokeId,
              openedAt: now,
              closedAt: null,
            });
          }
      ...
      grabHoldRef.current = null;
      const current = voiceGateRef.current;
      if (current?.mode === "scoped" && current.closedAt === null && !scopedKeyActiveRef.current) {
        closeVoiceGate("scoped");
      }
    }, 150);
    return () => clearInterval(timer);
  }, [closeVoiceGate, inputMode, openVoiceGate]);
...
type SemanticIntentResolutionContext = IntentResolutionContext & {
...
function resolveSemanticPlanAction(
...
function describeSemanticPlan(actions: SemanticPlanAction[]): string {
  if (actions.length > 1) {
...
function resolveIntentOperation(
...
function buildSemanticIntentContext(
  boardState: BoardState,
