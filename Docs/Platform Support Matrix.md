...
| Google Meet | Side-panel SDK initialization is real-client **V** for the publisher account; customer install still pending | `startActivity` and main-stage initialization are real-client **V** for the publisher account; participant starting-state join is **I** (implemented through versioned activity data; unverified with a second real participant) | **I** — real-client probe confirmed Meet does not delegate media permission to add-on iframes, so embedded delegated capture is dead in current Meet web. The Meet Media Bridge Chrome extension (explicit-start capture on the meeting origin, frames streamed into the add-on iframe) restores in-Meet gesture and passes emulated-bridge e2e; real-meeting verification pending. Companion window remains the no-extension fallback. Hidden capture rejected; Meet Media API stays Developer Preview and out of the public-beta path | **I** — voice via the Meet Media Bridge extension (explicit Start Airo; meeting-origin microphone streamed to the add-on frame and into the existing realtime transcription session; real-meeting verification pending); companion window without the extension; the deployed pilot has no transcription credentials configured. Meet Media API remains out of the public-beta path | Provider offers activity screen-sharing support; Airboard implementation is **NB** | Airboard transport is **P**; Meet lifecycle/activity resume is **NB** | **C** only after PF-002 proves meeting-context behavior; no promise today | Pointer/touchpad, keyboard, typed commands, and direct canvas controls remain accessible; the companion window provides gesture and voice when embedded capture is unavailable |
...
| Camera gestures | Candidate after manual validation | Via the Meet Media Bridge extension (explicit start, meeting-origin capture; real-meeting verification pending) — Meet confirmed it does not delegate media to add-on iframes; companion window without the extension; hidden capture rejected | Candidate via documented `devicePermissions` manifest media; PF-003 must prove desktop behavior | Candidate only if PF-004 proves Zoom webview capture; companion window otherwise | Companion window for gesture; pointer/touchpad plus typed command as the baseline |
