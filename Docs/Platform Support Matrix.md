...
| Google Meet | Google Workspace Marketplace Meet add-on | Meet desktop web, initially Chrome | Windows, macOS, ChromeOS | Side-panel/main-stage routes and an SDK-shaped adapter exist, but the adapter is a stub and has not run in Meet | First provider public-beta candidate |
...
| Google Meet | Side-panel SDK initialization is real-client **V** for the publisher account; customer install still pending | `startActivity` and main-stage initialization are real-client **V** for the publisher account; participant starting-state join is **NB** | **Blocked, release-critical**: the production Add-ons SDK exposes no Meet video track; cross-origin iframe access is prohibited; hidden/separate `getUserMedia` is rejected. Meet Media API is Developer Preview and not a public-beta dependency | **E** for the embedded beta: Meet owns microphone controls and the add-on starts no speech capture; Meet Media API is not a public-beta dependency | Provider offers activity screen-sharing support; Airboard implementation is **NB** | Airboard transport is **P**; Meet lifecycle/activity resume is **NB** | **C** only after PF-002 proves meeting-context behavior; no promise today | Pointer/touchpad, keyboard, typed commands, and direct canvas controls remain accessible but do not satisfy the gesture release gate |
...
| Camera gestures | Candidate after manual validation | Release-blocked: requires a GA Google-supported Meet self-video source; no hidden or separate Airboard capture | Candidate only if PF-003 proves Teams desktop media behavior | Candidate only if PF-004 proves Zoom desktop media behavior | Pointer/touchpad plus typed command remains accessible but is not feature parity |
| Microphone voice | Candidate after privacy, provider, and device validation | Excluded from the embedded Meet beta; Airboard does not start separate microphone capture | Candidate only if PF-003 proves permission and meeting-audio coexistence | Candidate only if PF-004 proves permission and meeting-audio coexistence | Typed command |
...
- `apps/web/app/meet/side-panel/page.tsx` and `apps/web/app/meet/main-stage/page.tsx` render Meet-shaped routes.
- `packages/integrations/src/googleMeetAdapter.ts` contains an SDK-shaped Google adapter, hard-coded identity, no activity-state join, no role/participant lifecycle, and no real-client evidence.
- `packages/integrations/src/index.ts` exports only standalone and Google Meet adapters; Teams and Zoom adapters do not exist.
- `apps/web/src/features/board/AirboardPrototype.tsx` directly requests camera/microphone on every surface and provides pointer/touchpad and typed-command paths.
