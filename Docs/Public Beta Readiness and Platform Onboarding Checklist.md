...
- [ ] **[BLOCKED] PF-002 — Run a Google Meet real-client feasibility spike.**
  ...
  - Blocker: The required production behavior—derive the signed-in user's gestures from Meet's existing camera stream without separate Airboard capture—is not available through the production Meet Add-ons SDK. Shipping the current no-media build would remove a core feature; restoring hidden or visible Airboard capture would violate the product requirement. Meet Media API cannot close the public-beta gate while it requires Developer Preview enrollment for every participant and restricted-scope verification. Customer-style Marketplace distribution, listing uploads, multi-user roles/devices, reconnect/state restoration, and the remaining matrix are also still outstanding. Test users and customers receive no Cloud IAM.
  - Next action: Keep Google Meet gesture support blocked from the production promise. Request Google Meet Media API Developer Preview access only for an isolated feasibility branch and obtain a Google product/partner answer on a GA self-video stream for add-ons. Resume production implementation only when Google provides a GA media contract suitable for arbitrary Marketplace users, or after an explicit product decision accepts a separate Airboard camera grant/companion capture. Do not submit the Marketplace listing as a gesture-enabled production app before this gate is resolved.
