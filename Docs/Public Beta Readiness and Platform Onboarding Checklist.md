...
- [ ] **[BLOCKED] PF-002 — Run a Google Meet real-client feasibility spike.**
  ...
  - Next action: Redeploy the pilot and verify the delegation probe in a real Meet meeting (does the add-on iframe delegate camera/microphone, and does embedded capture start without a second prompt?). Verify the companion-window flow in-meeting, including popup behavior from the add-on iframe. Configure `DEEPGRAM_API_KEY` on the pilot API before voice testing. Bring Teams' documented `devicePermissions` media path forward in PF-003. Do not submit the Marketplace listing until the delegation outcome, disclosures, and the remaining role/device matrix are verified.
